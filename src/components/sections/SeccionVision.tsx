...
export default function SeccionVision() {
  return (
    <Box w={'100%'} bg={'linear-gradient(180deg, rgba(26,54,93,1) 0%, rgba(15,31,54,1) 54%, rgba(0,0,0,1) 81%, rgba(0,0,0,0.577468487394958) 92%, rgba(0,0,0,0) 100%)'} color={'white'} id='vision'>
      <Container maxW={'6xl'} h={'inherit'} paddingBlock={'80px'}>
        <Flex gap={2} pos={'relative'} direction={{ base: 'column', lg: 'row' }} align={'center'}>
          ...
          <Box w={'400px'} flexShrink={0} >
            <Image w={'auto'} src='imgs/undraw_connected_world_wuay.svg' alt='Vision' />
          </Box>
        </Flex>

        <br />
        <Text fontSize={'3xl'} fontWeight={500} mb={'20px'} textAlign={{ base: 'center', lg: 'left' }}>Valores</Text>


        <Grid templateColumns={{ base: 'repeat(1, 1fr)', lg: 'repeat(5, 1fr)' }} gap={3}>
  ...
  return (
    <GridItem w='100%' >
      <Card h={'full'} bg={'blackAlpha.50'} color={'white'} border={'1px solid'} borderColor={'whiteAlpha.300'}>
        <CardHeader paddingBlock={2} textAlign={{ base: 'left', lg: 'center' }}>
          <Text fontSize={'xl'} fontWeight={500}>{valor}</Text>
        </CardHeader>
        <CardBody paddingBlock={2} textAlign={{ base: 'left', lg: 'center' }}>
          {children}
        </CardBody>
      </Card>
    </GridItem>
